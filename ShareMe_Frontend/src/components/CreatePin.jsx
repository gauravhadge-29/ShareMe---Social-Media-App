...
const CreatePin = ({user}) => {
  ...
  const uploadImage = (e) => {
    ...
    if (selectedFile.type === 'image/png' || selectedFile.type === 'image/svg' || selectedFile.type === 'image/jpeg' || selectedFile.type === 'image/gif' || selectedFile.type === 'image/tiff' || selectedFile.type === 'image/bmp') {
...
const savePin = ()=>{
  if(title && about && destination && imageAsset?._id && category){
    ...
    client.create(doc)
      ...
      .catch((err) => {
        console.log('Pin creation failed', err);
      });
  } else {
    setFields(true);
  ...
  return (
    <div className='flex flex-col justify-center items-center mt-5 lg:h-4/5'>
      ...
      <div className='flex flex-col lg:flex-row justify-center items-center bg-white lg:p-5 p-3 lg:w-4/5 w-full'>
                      ...
                      imageAsset ? (
                        ...
                        <label>
                          <div className='flex flex-col justify-center items-center '>
                            ...
                            <p className='mt-32 text-gray-400'>Use high-quality JPG, PNG, GIF, SVG, BMP or TIFF less than 20MB</p>
                            <input type="file" name='upload-image' onChange={uploadImage} className='w-0 h-0'/>
                          </div>
                        </label>
                      )
                    }{ wrongImageType && (
                      <div className='absolute bottom-3 left-3 p-3 rounded-md bg-red-500 text-white font-semibold'>
          ...
          <div className='flex flex-1 flex-col gap-6 lg:pl-5 mr-5 w-full'>
            <input
            type='text'
            value={title}
            ...
            <div className='flex flex-col'>
              <div>
                ...
                <select
                 ...
                 className='outline-none text-base sm:text-lg border-b-2 border-gray-200 p-2 w-4/5 rounded-md cursor-pointer'
                 name="" id="">
                  <option value="other" className='bg-white'>Select Category</option>
                  {categories.map((category)=>(
                    <option value={category.name} className='text-black text-base border-0 outline-none capitalize bg-white'>{category.name}</option>
                  ))}
                 </select>
              </div>
